...
int WriteVarInt(unsigned int x, unsigned char * buf)
{
    unsigned int y;
    int num_bytes, i, b;
...
/*---------------------------------------------------------------------------*/
void script_request_2( lua_State *L, char **buf, size_t *len )
{
    int pop = 1;
    lua_getglobal(L, "request");
    ...
    lua_call(L, 0, 1);
    const char *str = lua_tolstring(L, -1, len);
    *buf = realloc(*buf, *len + 5);
    pbuf = *buf;

    //memcpy(pbuf + 4, str, *len);

    int x = WriteVarInt(*len, pbuf);

    //uint16_t blen = htons(*len);
    //uint32_t blen = htons(*len);
...
*/
    ...
    *len = *len + x;
    memcpy(pbuf + x, str, *len);

    lua_pop(L, pop);
}
/*---------------------------------------------------------------------------*/
void script_http_response( lua_State *L, int status, buffer *headers, buffer *body )
{
...
/*---------------------------------------------------------------------------*/
int script_raw_response( lua_State *L, connection *c, int n )
{
    int status = 500;

    lua_getglobal(L, "response");
    lua_pushlstring(L, c->buf, n);
    lua_call(L, 1, 1);
...
/*---------------------------------------------------------------------------*/
size_t script_verify_http_request( lua_State *L )
{
...
/*---------------------------------------------------------------------------*/
void script_copy_value( lua_State *L_src, lua_State *L_dst, int index )
{
    switch( lua_type(L_src, index) )
    {
        case LUA_TBOOLEAN:
            lua_pushboolean(L_dst, lua_toboolean(L_src, index));
            break;
        case LUA_TNIL:
            lua_pushnil(L_dst);
            break;
        case LUA_TNUMBER:
            lua_pushnumber(L_dst, lua_tonumber(L_src, index));
            break;
        case LUA_TSTRING:
            lua_pushstring(L_dst, lua_tostring(L_src, index));
            break;
        case LUA_TTABLE:
            lua_newtable(L_dst);
            lua_pushnil(L_src);
            while( lua_next(L_src, index - 1) )
            {
                script_copy_value(L_src, L_dst, -2);
                script_copy_value(L_src, L_dst, -1);
            ...
            break;
        default:
            luaL_error(L_src, "cannot transfer '%s' to thread", luaL_typename(L_src, index));
    }
}
/*---------------------------------------------------------------------------*/
